...
var store = new MongoStore(function (ready) {
  MongoClient.connect(config.database.mongodb_account, function(err, db) {
        if (err) throw err;
 ...
 * Connect to MongoDB.
 */
mongoose.connect(config.database.mongodb_account);
mongoose.connection.on('error', function() {
  console.log('MongoDB Connection Error to account database. Please make sure that MongoDB is running.');
  process.exit(1);
});
...
app.set('ssl_port', process.env.SSL_PORT || 444);
...
/*
* Use API key to get service token
*/
var getServiceToken=true;
var serviceToken;
async.doWhilst(
    function() {return getServiceToken;},
    function (callback) {
        request('config.api_endpoints.service_token', function(err, response, body) {
            if (err) {
                logger.error(err);
            }
            if (response.statusCode != 200) {
                logger.error('Api call %s resulted in return code %s',config.api_endpoints.service_token,response.statusCode);
            }
            getServiceToken=false;
            logger.info(body)
            callback(null,body)
        });
    },
